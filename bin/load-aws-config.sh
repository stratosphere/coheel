...
echo -e "cluster setup:\n HDFS NameNode: $HDFS_NAME_NODE\n Flink JobManager: $FLINK_JOB_MANAGER:$FLINK_JOB_MANAGER_PORT"

export COHEEL_TOPIC_ARN="$(aws sns create-topic --name 'coheel' |  jd -r '.TopicArn')"
coheel_message() { aws sns publish --topic-arn $COHEEL_TOPIC_ARN --message target/run-output --subject "$1" ; }
