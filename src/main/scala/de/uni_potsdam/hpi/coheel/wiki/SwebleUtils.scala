...
object SwebleUtils {
	...
	class PlainTextConverter(private val config: SimpleWikiConfiguration, private var enumerateSections: Boolean, private var wrapCol: Int) extends Visitor {
		...
		private var sections: LinkedList[Integer] = _

		def this(config: SimpleWikiConfiguration, enumerateSections: Boolean) = {
			this(config, enumerateSections, Int.MaxValue)
		}
		def this(config: SimpleWikiConfiguration) {
			this(config, true)
		}
